...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var mm = new MatricesMultiplier();
            var mmp = new MatricesMultiplierParallel();

            //Act
            while(regularMultiplicationTime <= parallelMultiplicationTime)
            {
                matrixSize++;
                var firstMatrix = new Matrix(matrixSize, matrixSize, true);
                var secondMatrix = new Matrix(matrixSize, matrixSize, true);

                //checking how much time it takes with regular multiplier
                var stopwatch = Stopwatch.StartNew();
                mm.Multiply(firstMatrix, secondMatrix);
                regularMultiplicationTime = stopwatch.ElapsedMilliseconds;
                stopwatch.Reset();

                //checking how much time it takes with parallel multiplier
                stopwatch = Stopwatch.StartNew();
                mmp.Multiply(firstMatrix, secondMatrix);
                parallelMultiplicationTime = stopwatch.ElapsedMilliseconds;
                stopwatch.Reset();
            }
            //Assert
            //results were 27 19 36 29 27. Not sure for a better way to check efficiency and get exact number
            Assert.IsTrue(matrixSize > 10);
        }
